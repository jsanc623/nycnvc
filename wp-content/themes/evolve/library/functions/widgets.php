<?php
    ...
    //rewrite by denzel, does not know why evolve_get_option function fails here? 
    $options = get_option('evl_options');
    $evolve_layout         = (isset($options['evl_layout'])) ? $options['evl_layout'] : '2cr';
    ...
    if ( ( $evolve_layout == "2cr" || $evolve_layout == "2cl" || $evolve_layout == "3cr" || $evolve_layout == "3cl" || $evolve_layout == "3cm" ) ) {
        ...
        register_sidebar( array(
            'name'          => __( 'Sidebar Widgets Disabled', 'evolve' ),
            'id'            => 'sidebar-1',
            'before_widget' => '<div id="%1$s" class="widget %2$s"><div class="widget-content">',
            'after_widget'  => '</div></div>',
            'before_title'  => '<div class="before-title"><div class="widget-title-background"></div><h3 class="widget-title">',
            'after_title'   => '</h3></div>',
            'description'   => __( 'You are using one column layout. Please visit theme settings page and change layout to enable sidebar widgets. Widgets placed on this area won\'t be active.', 'evolve' ),
        ) );
    }
